 ...
 *
 * The command IS the tool name. `refund_transaction` runs as
 * `refund-transaction`, and the underscore form works too. Inventing a prettier
...
type Flag = {
  /** The schema key, e.g. `item_name`. */
  key: string;
  /** The long flag, e.g. `--item-name`. */
  flag: string;
  kind: FlagKind;
  required: boolean;
  repeatable: boolean;
...
function kindOf(schema: ZodTypeAny): { kind: FlagKind; choices?: string[]; repeatable: boolean } {
  ...
  switch (typeName) {
    ...
    case "ZodArray": {
      // An array of scalars is repeatable (`--text a --text b`). An array of
      ...
      // Treating it as JSON meant `--status refunded` was rejected and you had
      // to write `--status '"refunded"'` instead.
      const scalar =
...
export function parseArgs(argv: string[], flags: Flag[]): Record<string, unknown> {
  ...
  // One bare argument fills the first required flag, so `get-product "abc"`
...
export function exitCodeFor(error: unknown): number {
  ...
  // Config before auth: "no cart configured" mentions an API key, and matching
  // auth first sent someone who had configured nothing looking for a revoked
  // credential. Only when there is no HTTP status, so a real 401 still wins.
  if (!status && /not configured|no [a-z ]*(account|cart|credential|token|key)s? (is |are )?configured|missing .*env/.test(text))
    return EXIT.config;
  if (status === 401 || status === 403 || /auth|credential|token|api key/.test(text)) return EXIT.auth;
  if (status === 404 || /not found/.test(text)) return EXIT.notFound;
  // A refused write is the caller getting the invocation wrong, not the API
  // failing: no --confirm, READ_ONLY, or destructive writes switched off.
...
/**
 * `--select id,customer.email` keeps only the named fields. Dotted paths
 * descend, arrays are traversed element-wise. This is what makes a long
 * transaction list affordable.
 */
export function selectFields(data: unknown, paths: string[]): unknown {
  ...
  // `--select orders.id,orders.total` returned only the total, silently.
 ...
 *
 * Reading tools return data rather than rendered text, so JSON is the honest
 * default for them. A handler that does return a string prints it as-is.
 * `--json` forces JSON either way so a script never has to know which kind of
 * tool it called.
 */
 ...
 *
 * The package ships two binaries onto the same file. Printing `thrivecart-mcp`
 * at someone who typed `thrivecart-cli` hands them a command that works but is
 * not the one they have in their fingers.
 */
function binName(): string {
  const name = (process.argv[1] ?? "").split("/").pop() ?? "";
  return name.startsWith("thrivecart-cli") ? "thrivecart-cli" : "thrivecart-mcp";
}
...
export async function runCli(argv: string[]): Promise<number> {
  ...
  if (!spec) {
    ...
    emitError(
      new Error(
        hidden
          ? `${name} is unavailable: this server is running with THRIVECART_READ_ONLY=1.`
